...
class SemanticMemoryService:
    ...
    def get_user_persona(self, user_id: str) -> Optional[UserPersona]:
        ...
        if persona and REDIS_CACHE_AVAILABLE:
            # Cache for future use
            cache_persona(user_id, persona.to_dict())
        
        return persona
    
    def update_user_persona(self, user_id: str, **updates) -> Optional[UserPersona]:
        """Update user persona and invalidate cache"""
        persona = self.persona_repo.get_by_user_id(user_id)
        if not persona:
            return None
